  ...
  const schema = yup.object().shape({
    ...
    aspects: yup
      ...
      .required('At least one aspect is required'),
    imageUrls: yup
      .array()
      .of(yup.string().url('Must be a valid URL'))
      .required('Image URLs are required'), // Keep it required if it's necessary
    condition: yup.string().required('Condition is required'), // Make sure this field is also included
  });
  ...
  const EbayListing = () => {
    const {
      ...
      defaultValues: {
        aspects: [{ name: '', value: '' }],
        imageUrls: [], // Initialize with a placeholder or empty
        condition: 'NEW', // Set a default value if necessary
      },
    });
